...
class extendedTest extends TestCase
{
    ...
    function soapFaultstring()
    {
        ...
        try {
            ...
            static::assertTrue(isset($previousException->faultstring) && ! empty($previousException->faultstring) && preg_match("/unauthorized/i",
                    $e->getMessage()));
        }
    }
    ...
    function soapAuthErrorInitialSoapFaultsNoWsdl()
    {
        ...
        $this->CURL->setAuthentication("fail", "fail");
        try {
            $wsdl = $this->CURL->doGet('https://test.resurs.com/ecommerce-test/ws/V4/SimplifiedShopFlowService',
    ...
    function soapAuthErrorNoInitialSoapFaultsNoWsdl()
    {
        ...
        $this->CURL->setAuthentication("fail", "fail");
        try {
            $this->CURL->doGet('https://test.resurs.com/ecommerce-test/ws/V4/SimplifiedShopFlowService',
                NETCURL_POST_DATATYPES::DATATYPE_SOAP);
        } catch (\Exception $e) {
    ...
    function rbSimpleXml()
    {
        try {
            ...
            $wsdlResponse = $this->CURL->doGet($this->wsdl,
                NETCURL_POST_DATATYPES::DATATYPE_SOAP_XML)->getPaymentMethods();
        } catch (\Exception $e) {
